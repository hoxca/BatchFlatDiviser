function fileutils(filePath,imageWindow) {
   ...
   this.writeImage = function(imageWindow,filePath)
   {
      ...
      var fileDir = (flatDiviser.outputDirectory.length > 0) ? flatDiviser.outputDirectory :
                    File.extractDrive( filePath ) + File.extractDirectory( filePath );
      if ( !fileDir.endsWith( '/' ) )
         fileDir += '/';
      var fileName = File.extractName( filePath );
      var outputFilePath = fileDir + flatDiviser.outputPrefix + fileName + flatDiviser.outputPostfix + flatDiviser.outputExtension;

      console.writeln("<end><cbr><br>Output file:");

      if ( File.exists(outputFilePath) )
      {
         if ( flatDiviser.overwriteExisting )
         {
